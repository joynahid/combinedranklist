...
from robots.codeforces.model.structure import (
    Contest,
    ProblemResults,
    Problems,
    Rows,
...
class Codeforces:
    ...
    async def standings(contest_ids, handles=None):
        ...
        for response in responses:
            ...
            problems = [
                toNamedTuple(Problems, problem_dict)
                for problem_dict in response["problems"]
            ]

            for row in response["rows"]:
                row["party"]["members"] = [
                    toNamedTuple(Member, member) for member in row["party"]["members"]
                ]
                row["party"] = toNamedTuple(Party, row["party"])
                row["problemResults"] = [
                    toNamedTuple(ProblemResults, problem_result)
